...
说明：
- `pre-commit` 会自动扫描暂存区，阻止 `APP Secret / API Key` 明文进入仓库。
- `pre-push` 会自动执行本地 `Secret Scan + Secret Leak Scan`（含 repo/history）后才允许推送。
- CI 会在 PR / push 时执行 `gitleaks` + `scan-repo`（`.github/workflows/secret-scan.yml`、`.github/workflows/secret-leak-scan.yml`）。
...
- `pre-push` runs local `Secret Scan + Secret Leak Scan` (repo/history) before allowing push.
- CI runs `gitleaks` + `scan-repo` on push and pull requests (`.github/workflows/secret-scan.yml`, `.github/workflows/secret-leak-scan.yml`).
...
├── .github/workflows/secret-scan.yml # CI gitleaks scan
├── .github/workflows/secret-leak-scan.yml # CI repo/history secret scan
